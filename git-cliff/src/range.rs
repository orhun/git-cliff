...
pub(crate) fn transform_range(
    ...
    if let Some(range) = args.range.as_deref() {
        return Ok(parse_legacy_range(range));
    }
    if args.unreleased {
...
/// Map a positional range string onto a `CommitRange`, following git's own
/// semantics for each form: `A..B` is `(A, B]`; an empty side defaults to
/// HEAD (`A..` walks to HEAD, `..B` starts after HEAD); a lone revspec `R`
/// bounds the range on the right, `[<first-commit>, R]` (the bare-SHA form
/// the walker historically supported, widened to any revspec). The revs are
/// resolved later, so an unresolvable token fails there instead of being
/// silently dropped.
fn parse_legacy_range(range: &str) -> CommitRange {
    match range.split_once("..") {
        Some((left, right)) => CommitRange {
            left: Some(Endpoint::exclusive(if left.is_empty() {
                "HEAD"
            } else {
                left
            })),
            right: (!right.is_empty()).then(|| Endpoint::inclusive(right)),
        },
        None => CommitRange {
            left: None,
            right: Some(Endpoint::inclusive(range)),
        },
    }
}
...
mod tests {
    ...
    #[test]
    fn parse_legacy_range_defaults_empty_left_endpoint_to_head() {
        // git's `..B` means `HEAD..B`.
        let range = parse_legacy_range("..v2.0");
        let left = range.left.expect("left defaults to HEAD");
        assert_eq!(left.rev, "HEAD");
        assert!(!left.inclusive);
        let right = range.right.expect("right endpoint");
        assert_eq!(right.rev, "v2.0");
        assert!(right.inclusive);
    }

    #[test]
    fn parse_legacy_range_open_right_endpoint_walks_to_head() {
        // `v1.0..` is a documented form (git's `v1.0..HEAD`): exclusive
        // left, default right.
        let range = parse_legacy_range("v1.0..");
        let left = range.left.expect("left endpoint");
        assert_eq!(left.rev, "v1.0");
        assert!(!left.inclusive);
        assert!(range.right.is_none());
    }

    #[test]
    fn parse_legacy_range_single_token_is_right_inclusive() {
        // A positional without `..` bounds the range on the right, walking
        // from the first commit: main's bare-SHA semantics (root..SHA),
        // widened to any revspec. An unresolvable token then fails loudly
        // at endpoint resolution instead of being silently dropped.
        let range = parse_legacy_range("v2.0");
        assert!(range.left.is_none());
        let right = range.right.expect("right endpoint");
        assert_eq!(right.rev, "v2.0");
